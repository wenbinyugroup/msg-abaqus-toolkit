...
thisDir = os.path.join(os.path.dirname(thisDir), 'Image')
...
class SG2DV5DB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        AFXDataDialog.__init__(self, form, '2D Structure Genome - Unit Cell',
        ...
        HFrame_2 = FXHorizontalFrame(p=GroupBox_1, opts=0, x=0, y=0, w=0, h=0,
            pl=0, pr=0, pt=0, pb=0)
        FXRadioButton(p=HFrame_2, text='Square', tgt=form.profileKw, sel=1)
        FXRadioButton(p=HFrame_2, text='Hexagonal', tgt=form.profileKw, sel=2)
        ...
        GroupBox_5 = FXGroupBox(p=HFrame_6, text='Fiber', opts=FRAME_GROOVE)
        ...
        AFXNote(p=self.swt_vf, message='0 < vf_f+vf_i <= 0.78')
        AFXNote(p=self.swt_vf, message='0 < vf_f+vf_i <= 0.90')
        ...
        msgHandler8 = type(self).__name__.split('.')[-1] + '.onComboBox_8MaterialsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount, msgHandler8) )

        msgHandler6 = type(self).__name__.split('.')[-1] + '.onComboBox_6MaterialsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount+1, msgHandler6) )

        msgHandler5 = type(self).__name__.split('.')[-1] + '.onComboBox_5MaterialsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount+2, msgHandler5) )

        # Materials combo
        #
        self.ComboBox_8 = AFXComboBox(p=VAligner_3, ncols=0, nvis=1, text='Fiber: ', tgt=form.fiber_matnameKw, sel=0)
        ...
        self.ComboBox_5 = AFXComboBox(p=VAligner_3, ncols=0, nvis=1, text='Interphase: ', tgt=form.interface_matnameKw, sel=0)
        ...
        fileName = os.path.join(thisDir, 'squareInterface.png')
        self.icon_sqr = afxCreatePNGIcon(fileName)
        FXLabel(p=self.swt_fig, text='', ic=self.icon_sqr)
        
        fileName = os.path.join(thisDir, 'HexInterface.png')
    ...
    def updateComboBox_8Materials(self):
        ...
        names = list(mdb.models[modelName].materials.keys())
        names.sort()
        for name in names:

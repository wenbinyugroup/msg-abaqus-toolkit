...
class SG1D_v3DB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        msgHandler = type(self).__name__.split('.')[-1] + '.onComboBox_1MaterialsChanged'
        ...
        msgHandler = type(self).__name__.split('.')[-1] + '.onComboBox_3PartsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount, msgHandler) )
        ...
        msgHandler = type(self).__name__.split('.')[-1] + '.onComboBox_4LayupsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount, msgHandler))
        ...
        msgHandler = type(self).__name__.split('.')[-1] + '.onComboBox_sSectionsChanged'
        exec('FXMAPFUNC(self, SEL_COMMAND, AFXDataDialog.ID_LAST+%d, %s)' % (msgCount, msgHandler) )
    ...
    def updateComboBox_3Parts(self):
        ...
        names = list(mdb.models[modelName].parts.keys())
        names.sort()
        for name in names:
    ...
    def updateComboBox_4Layups(self):
        ...
        #
        self.ComboBox_4.clearItems()
        names = list(mdb.models[modelName].parts[partName].compositeLayups.keys())
        names.sort()
        for name in names:
    ...
    def updateComboBox_sSections(self):
        ...
        names = []
        for name, section in list(mdb.models[modelName].sections.items()):

...
#                    print ma
#                    print abq_elset
                    if ma not in list(abq_elset.keys()):
                        abq_elset[ma] = []
    #                else:
    #                    flag = 1
                    if ma not in list(layer_type.keys()):
                        k = len(list(layer_type.keys())) + 1
                        j = ma.rfind('_')
                        m = ma[:j]
                        a = ma[j+1:]
                        if 'p' in a:
...
#                    milestone(first)
                    ...
                    for temp in line:
                        if 'name' in temp:
                            ...
                            if not distr_name in list(distribution.keys()):
                    ...
                    for temp in line:
                        if 'name' in temp:
                            ...
                            if not orien_name in list(orientation.keys()):
...
#                    milestone(first)
                    ...
                    mn = line[mi].split('=')
                    mn = mn[-1].strip()
                    if mn not in list(material.keys()):
                        mid = len(list(material.keys())) + 1
                        material[mn] = [mid]
                    continue
                elif first == '*Density':
#                    milestone(first)
...
#                    material[mn].append(density)
        ...
        #                print eid
                        ...
                        if eid not in list(distr_list.keys()):
                            if nsg == 2:
...
#    print 'Finish reading Abaqus inputs.'
    ...
    for k in list(abq_elset.keys()):
        if k not in used_section:
            del abq_elset[k]
    
    eid_maid = {}
#    print abq_elset
    for k, v in list(abq_elset.items()):
        maid = layer_type[k][0]
        for eid in v:
...
#    print 'Writing SwiftComp inputs...'
    ...
    with codecs.open(out_file_name, encoding = 'utf-8', mode = 'w') as fout:
        ...
        if trans_flag == 1:
            ...
            for e, abc in list(distr_list.items()):
                writeFormat(fout, 'd'+'E'*9, [int(e)]+abc)
        fout.write('\n')
        
        # print 'Writing layer types...'
        if nlayer != 0:
            for v in list(layer_type.values()):
                mn = v[1]
                mid = material[mn][0]
                v[1] = mid

...
for i in range(ln, ln + nelem):
    ...
    if section_id not in list(sections.keys()):
        sections[section_id] = []
...
for i in range(nmate):
    ...
    if mate_type == 0:
        # Isotropic
        elastic = list(map(float, all_lines[ln+2].split()))
        ln = ln + 3
    elif mate_type == 1:
        # Engineering Constants
        elastic = list(map(float, all_lines[ln+2].split()))
        elastic.append(list(map(float, all_lines[ln+4].split())))
        elastic.append(list(map(float, all_lines[ln+3].split())))
        ln = ln + 5
    elif mate_type == 2:
        # Anisotropic
        ln = ln + 8
